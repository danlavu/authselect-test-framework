...
class LocalUser(GenericUser):
    ...
    def delete(self) -> None:
        ...
        self.util.host.conn.run(
            f"userdel '{self._name}' --force --remove",
            log_level=ProcessLogLevel.Error,
        )
        if self._name in self.util._users:
            self.util._users.remove(self._name)

    def discard(self) -> None:
        """
        Remove the local user if they exist.
        """
        self.util.logger.info(f'Discarding local user "{self._name}" on {self.util.host.hostname}')
        self.util.host.conn.run(
            f"userdel '{self._name}' --force --remove",
...
class LocalGroup(GenericGroup):
    ...
    def delete(self) -> None:
        ...
        self.util.host.conn.run(
            f"groupdel '{self._name}' -f",
            log_level=ProcessLogLevel.Error,
        )
        if self._name in self.util._groups:
            self.util._groups.remove(self._name)

    def discard(self) -> None:
        """
        Remove the local group if it exists.
        """
        self.util.logger.info(f'Discarding local group "{self._name}" on {self.util.host.hostname}')
        self.util.host.conn.run(
            f"groupdel '{self._name}' -f",
            raise_on_error=False,
            log_level=ProcessLogLevel.Error,
