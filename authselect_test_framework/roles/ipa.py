...
class IPADNSZone(IPADNSServer, GenericDNSZone):
    ...
    def delete_record(self, name: str) -> None:
        ...
        The DNS entry contains sshfp records that are not removed leading to need to determine record type.
        ...
        dig_records = self.host.conn.run(f"dig +short +norecurse {name} '@{self.server}'").stdout_lines
        dig_records = [s.rstrip("\r") for s in dig_records]
        zones: list[str] = []
        for record in dig_records:
            if self.host.conn.run(f"ipa dnszone-show {ip_to_ptr(record)}"):
                zones.append(ip_to_ptr(record))

        for zone in [self.zone_name] + zones:
            zone_records = self.host.conn.run(f"ipa dnsrecord-find {zone} --raw").stdout_lines
            zone_records = [s.rstrip("\r") for s in zone_records]
            if name in zone_records:
                if len(record.split(":")) == 1:
                    record_type, data = record.split(":")
                    if record_type.trim() == "arecord":
                        self.host.conn.run(f"ipa dnsrecord-del {self.zone_name} --a-rec={record} {data}")
                    elif record_type.trim() == "aaaa":
                        self.host.conn.run(f"ipa dnsrecord-del {self.zone_name} --aaaa-rec={record} {data}")
                    elif record_type.trim() == "ptr":
                        self.host.conn.run(f"ipa dnsrecord-del {self.zone_name} --srv-rec={record} {data}")
                    elif record_type.trim() == "naptr":
                        self.host.conn.run(f"ipa dnsrecord-del {self.zone_name} --naptr-rec={record} {data}")
